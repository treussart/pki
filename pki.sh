...
CN_CERT=""
CN_CERT_WEB="$name"" Web"
CN_CERT_EMAIL="$name"" Email"
EXTENSIONS=""
EXTENSIONS_WEB="web.extensions.ini"
...
bundle() {
    ...
    cat "$dest_key""$sld""$tld"-"$TYPE".key "$dest_cert""$TYPE"."$sld""$tld"-bundle.pem > "$dest_key""$TYPE"."$sld""$tld"-bundle-all.pem
}
